...
exports.createServer = function( context, modules ){
      ...
      (function(m, mm, module){
        meryl.h('GET /' + module + '/' + m + '(/<resource>)?', function () {
          ...
          if(async){
            ...
            mm[m].apply(context, args);
            
          }
          else {

            modules[module].views[self.responseFormat](args, mm[m].apply(context, args), function(type, body) {
              self.send(body);
            })
